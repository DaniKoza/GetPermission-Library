...
public class GrantMe {
    private final Activity activity;
    private final String TAG = getClass().getSimpleName();
    private final int REQUEST_CODE = 2901;
    private final int SETTING_REQUEST_CODE = 1803;
    private ResponseListener resListener;
    private String[] permissionsForResults;
    private final MySharedPrefs msp;
    private boolean debug;


    public GrantMe(Activity activity, ResponseListener resListener) {
        this(activity);
    ...
    public GrantMe(Activity activity) {
        this.activity = activity;
        msp = new MySharedPrefs(activity);
        debug = false;
            ...
            if (checkPermissionsList(permissionsForResults)) {
                ...
                for (String permission : permissionsForResults) {
                    ...
                    if (neverAskAgain.size() > 0) {
                        ...
                        resListener.onNeverAskAgain(neverAskAgain.toArray(new String[0]));
                    }
                    if (notGranted.size() > 0) {
                        logger("the permissions " + notGranted + " not granted");
                        resListener.onNotGranted(notGranted.toArray(new String[0]));
                    }
                    logger("all granted? false");
                    resListener.onGranted(false);
                }
            }
            return true;
        }
    ...
    public boolean onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == REQUEST_CODE) {
            ...
            resListener.onGranted(allGranted);
            return true;
        }
    ...
    public void askPermissionsFromSetting(String msg, String[] permissions, @NonNull ResponseListener resListener, DialogListener dialogListener) {
        setResponseListener(resListener);
     ...
     *
     * @param permissions    - String array of permissions
     * @param resListener    - ResponseListener callbacks to perform after onRequestPermissionsResult - set as default
     * @param msg            - String: message that will show to the user. here you can explain why you need the permissions
     * @param dialogListener - DialogListener: callback to perform on buttons click
     */
     ...
     *
     * @param permissions    - String array of permissions
     * @param resListener  - GrantListener callbacks to perform after onRequestPermissionsResult- set as default
     * @param title          - String: dialog title
     * @param msg            - String: message that will show to the user. here you can explain why you need the permissions
     * @param dialogListener - DialogListener: callback to perform on buttons click
     */
    public void requestPermissionsWithDialog(@NonNull String[] permissions, @NonNull ResponseListener resListener, String title, String msg, DialogListener dialogListener) {
        setResponseListener(resListener);
     ...
     *
     * @param permissions   - String array of permissions
     * @param resListener - ResponseListener callbacks to perform after onRequestPermissionsResult
     */
    ...
    /**
     * request permissions from the user. using the default ResponseListener.
     * this method not handling "don't ask me again" but if permission mark with this flag you will
     * get the permission in the onNeverAskAgain method in  ResponseListener callback
     *
     * @param permissions - String array of permissions
     */
    ...
    /**
     * set the default ResponseListener
     */
    public void setResponseListener(ResponseListener resListener) {
        this.resListener = resListener;
        logger("new ResponseListener was set");
    }
    ...
    /**
     * check if the were permissions asked in the past
     *
     ...
     * @return the permissions that not granted yet
     */
